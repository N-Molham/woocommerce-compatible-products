...
// item quantity
c.raw_qty=p.val(),c.raw_qty=parseFloat(c.raw_qty>0?c.raw_qty:"0"),0===c.raw_qty&&t&&(
// use current configuration quantity
c.raw_qty=t.quantity,p.val(c.raw_qty),
// trigger calculator change
g.trigger("wc-measurement-price-calculator-update")),
// item price
c.price=o.find(".product_price .amount").text(),c.raw_price=parseFloat(c.price.replace(wc_compatible_products_params.woocommerce_currency_symbol,"")),c.total=c.raw_price,
...
for(var h=0,i=a.product_variations.length;i>h;h++){var j=a.product_variations[h];if(j.variation_id.toString()===k.val()){
// set product name
c.name=j.variation_name;break}}}else
// other parts ( fittings )
//noinspection JSDuplicatedDeclaration
for(var h=0,i=u.length;i>h;h++){var l=u[h];"variation_id"in a&&"variation_id"in l&&a.variation_id!==l.variation_id||l.product_id===a.product_id&&(
// setup item data
...
// vars
var h=a(b),i={},j=g.data(),k=g.find("input[name=variation_id]"),l=g.find(".variations"),m=g.find(".wc-cp-assemblies-subtotal-amount"),n=a("#measuring-instructions-button").removeClass("hidden"),o=a("#price_calculator"),p=o.find("#length_needed"),q=location.search.indexOf("wc-cp-need-fittings=yes")>-1,r=location.search.indexOf("wc_cp_edit_assembly=yes")>-1,s=null,t=null,u=null;wc_compatible_products_params.is_assembly_product_page&&(q=!0),
// when calculator amount field on focus
p.on("focus",function(a){a.target.select()}),
...
g.find(".wc-cp-products-list").trigger("wc-cp-change-state"),t&&"parts"in t&&t.parts.length){
// sort parts by box order
var i=t.parts.sort(function(a,b){return a.box_order>b.box_order?1:b.box_order>a.box_order?-1:0});for(p=0,q=i.length;q>p;p++){
// vars
var k=i[p],l=k.variation_id?k.variation_id:k.product_id,n=b;
// find related add-to-assembly button
c!==b&&l==b.data("product")||(
// query it instead
n=g.find('.compatible-product-add-to-cart-link[data-product="'+l+'"]:not(.disabled):first()')),n.length&&
// trigger box state change
n.closest(".wc-cp-products-list").trigger("wc-cp-change-state",[n]),
// get item setup
e.push(f(k))}}
// get main product item setup0
e.push(f(j,!0));for(var o=[],p=0,q=e.length;q>p;p++){var r=e[p];r.is_assembly&&(
...
g.on("wc-measurement-price-calculator-update",function(){t&&(i.calc_update&&i.calc_update.abort(),i.calc_update=a.post(wc_add_to_cart_params.ajax_url,{action:"update_assembly_amount",amount:p.val(),assembly_key:t.key,security:wc_compatible_products_params.assembly_quantity_nonce},function(a){"success"in a&&(a.success?
// trigger assembly configuration update
g.trigger("wc-cp-update-assembly-config"):alert(a.data))},"json"))}),
// move button to new location
a('<tr><td colspan="2"></td></tr>').insertAfter(p.closest("tr")).find("td").append(n),
...
// initialize popovers
g.find(".compatible-product-link").popover(),u=g.find(".wc-cp-products-list").data("products"),h.trigger("vc_reload"),t=g.find(".wc-cp-assembly-config").data("config"),t&&(p.val(t.quantity),g.trigger("wc-measurement-price-calculator-update"),t.parts&&t.parts.length&&(q=!0)),q&&(g.find(".wc-cp-need-compatible").prop("checked",!0),q=!1),g.find(".wc-cp-need-compatible").trigger("wc-cp-change"),g.trigger("wc-cp-update-assembly-config"),r&&(l.addClass("hidden"),g.find(":input:submit").addClass("update-assembly").text(wc_compatible_products_params.labels.edit_assembly).parent().append('<input type="hidden" name="wc_cp_update_assembly" value="'+t.key+'" />'))}),
...
// json response
a.success?(t=a.data,g.trigger("wc-cp-update-assembly-config",[c])):(c.button("reset"),console.log(a.data)):(c.button("reset"),console.log(a))},"json")}}),
// add product to cart click
...
b.prop("disabled",!0),
// additional props
c.action="remove_compatible_product_from_assembly",c.security=wc_compatible_products_params.assembly_remove_nonce,c.assembly_key=t.key,a.post(wc_add_to_cart_params.ajax_url,c,function(a){"success"in a?a.success?(t=a.data,g.trigger("wc-cp-update-assembly-config")):alert(a.data):console.log(a)},"json").always(function(){
// re-enable button
b.prop("disabled",!1)})}),
...
var v=a(".woocommerce-message");v.length&&setTimeout(function(){v.animate_scroll_to(180)},100)}}),
